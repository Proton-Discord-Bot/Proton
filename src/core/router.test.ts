...
test('routes autocomplete to the command autocomplete handler', async () => {
  ...
  const cmd = {
    data: { name: 'ping' },
    run: async () => {},
    autocomplete: async (ctx: unknown) => {
      calledWith = ctx;
    },
  };
  const interaction: any = {
...
test('passes the decoded action and args to a feature-level component', async () => {
  let seen: { action?: string; args?: string[] } = {};
  const component = {
    match: 'info',
    run: async (ctx: any) => {
      seen = { action: ctx.action, args: ctx.args };
    },
  };
  const client: any = {
    commands: new Map(),
    components: new Map([['info', component]]),
    db: {},
    translator: () => (k: string) => k,
  };
  const interaction: any = {
  ...
  expect(seen.action).toBe('kick');
  expect(seen.args).toEqual(['12345']);
});
