...
test('warnsRepo.clear removes only the requested amount when given a limit', () => {
  const db = createDb(':memory:');
  migrateDb(db);
  const w = warnsRepo(db);
  w.add('g1', 'u1', 'a');
  ...
  expect(w.clear('g1', 'u1', 2)).toBe(2);
  expect(w.count('g1', 'u1')).toBe(1);
});

test('warnsRepo.clear with a limit larger than the row count clears what exists', () => {
  const db = createDb(':memory:');
  migrateDb(db);
  const w = warnsRepo(db);
  w.add('g1', 'u1', 'a');

  expect(w.clear('g1', 'u1', 10)).toBe(1);
  expect(w.count('g1', 'u1')).toBe(0);
});

test('warnsRepo.clear leaves other users untouched', () => {
  const db = createDb(':memory:');
  migrateDb(db);
  const w = warnsRepo(db);
  w.add('g1', 'u1', 'a');
